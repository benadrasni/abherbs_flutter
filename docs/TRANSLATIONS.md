...
Retired `{lang}-GT` (official catalog is full-7; do not recreate): `de-GT` (2026-09-09), `pl-GT` and `ru-GT` (2026-09-10), `es-GT` and `pt-GT` (2026-09-12), `ja-GT` (2026-09-13), `it-GT`, `nl-GT` and `uk-GT` (2026-09-14), `hu-GT` and `da-GT` (2026-09-17), `sv-GT` (2026-09-17), `no-GT` and `fi-GT` (2026-09-18). App `languageUsesGoogleTranslate` and website `LANGUAGES_WITHOUT_GT` skip `en sk de fr cs pl ru es pt ja it nl uk hu da sv no fi`.
...
12. `hu` — pipeline `/translate-hu`: seven fields and herbalism from live English; trivia recurate for Hungary (Év fája, Év vadvirága; drop UK county / US state / Hungarian megye or city flowers; Hungary has no official national flower). Style: `.grok/skills/translate-hu/references/STYLE.md`. Morphological terms: Wikipedia Virágzat (`hu_wikipedia_glossary`). Do not copy Slovak, Czech, German, or Polish body. Catalog full-7 (verified 2026-09-16); language pass `_hu_lang_pass_1420` applied 2026-09-16. `hu-GT` deleted 2026-09-17.
13. `da` — pipeline `/translate-da`: seven fields and herbalism from live English; trivia recurate for Denmark (Årets Danske Urt; drop UK county / US state / Danish kommune or city flowers; Denmark has no official national flower). Style: `.grok/skills/translate-da/references/STYLE.md`. Morphological terms: Wikipedia Botaniske fagudtryk (`da_wikipedia_glossary`). Danish vernaculars: Arter.dk (`arter_dk`), Naturbasen (`naturbasen`). Encyclopedia: Lex (`lex_dk`). Do not copy Swedish, Norwegian, German, or Dutch body. Catalog full-7 (two hybrids filled 2026-09-17); `da-GT` deleted 2026-09-17.
14. `sv` — pipeline `/translate-sv`: seven fields and herbalism from live English; trivia recurate for Sweden (drop UK county / US state / Swedish landskapsblommor or kommun flowers; Sweden has no official national flower and no yearly wild-plant title). Style: `.grok/skills/translate-sv/references/STYLE.md`. Morphological terms: Wikipedia Botanisk ordlista (`sv_wikipedia_glossary`). Swedish vernaculars: Artfakta (`artfakta`), Den virtuella floran (`virtuella_floran`). Do not copy Danish, Norwegian, German, or Dutch body. Catalog full-7 (two hybrids filled 2026-09-17); language pass `_sv_lang_pass_1420` applied 2026-09-17; `sv-GT` deleted 2026-09-17.
15. `no` — pipeline `/translate-no`: seven fields and herbalism from live English; trivia recurate for Norway (Årets villblomst; drop UK county / US state / Norwegian kommune or fylke flowers; Norway has no official national flower). Style: `.grok/skills/translate-no/references/STYLE.md`. Morphological terms: Wikipedia Blomsterstand (`no_wikipedia_glossary`); UiO Botanisk- og plantefysiologisk leksikon (`uio_leksikon`). Norwegian vernaculars: Artsdatabanken (`artsdatabanken`). Encyclopedia: Store norske leksikon (`snl`). Catalog code is `no` (app `nb` → `no`); write Bokmål, not Nynorsk. Do not copy Danish, Swedish, German, or Dutch body. Catalog full-7; language pass `_no_lang_pass_1420` applied 2026-09-18; `no-GT` deleted 2026-09-18.
16. `fi` — pipeline `/translate-fi`: seven fields and herbalism from live English; trivia recurate for Finland (drop UK county / US state / Finnish maakuntakasvit or kunta flowers; Finland has no official national flower and no yearly wild-plant title). Style: `.grok/skills/translate-fi/references/STYLE.md`. Morphological terms: Tieteen termipankki Kasvitiede (`tieteentermipankki`); Wikipedia Kukinto (`fi_wikipedia_glossary`). Finnish vernaculars: Laji.fi (`laji_fi`). Finnish body: LuontoPortti Finnish pages (`luontoportti_fi`). Do not copy Danish, Swedish, Norwegian, German, Dutch, or Estonian body. Catalog full-7; `fi-GT` deleted 2026-09-18.
17. `et` — pipeline `/translate-et`: seven fields and herbalism from live English; trivia recurate for Estonia (rukkilill as rahvuslill on *Centaurea cyanus*; Aasta orhidee and catalog Aasta puu; drop UK county / US state / Estonian maakond or linn flowers; no aasta taim / aasta lill wild-herb title). Style: `.grok/skills/translate-et/references/STYLE.md`. Morphological terms: Sõnaveeb Botaanika terminibaas (`sonaveeb_bot`); Wikipedia Õisik (`et_wikipedia_glossary`). Estonian vernaculars: Eestikeelsete taimenimede andmebaas (`taimenimed`, https://taimenimed.ut.ee/). Estonian occurrence: eElurikkus (`elurikkus`). Do not copy Finnish, Danish, Swedish, Norwegian, German, or Dutch body. Do not start a full-catalog pass until asked.
18. The rest of the UI set
...
- Skills: `/translate-de`, `/translate-sk`, `/translate-fr`, `/translate-cs`, `/translate-pl`, `/translate-ru`, `/translate-es`, `/translate-pt`, `/translate-ja`, `/translate-it`, `/translate-nl`, `/translate-uk`, `/translate-hu`, `/translate-da`, `/translate-sv`, `/translate-no`, `/translate-fi` (copies in `ingest/skills/`)
