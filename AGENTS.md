...
- Never translate an English common name into another language. Vernacular `label` / `names` must come from a source in that language (Wikidata, that Wikipedia title, EPPO Global Database https://gd.eppo.int/, GBIF vernacular names https://www.gbif.org/, a flora such as BOTANY.cz, or for Slovak Kultúra slova or Slovenské botanické menoslovie 2025). If none exists, omit `label` and keep the Latin name.
